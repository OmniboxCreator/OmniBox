...
static void test_crc_vector(void)
{
  ...
  CHECK(pt_crc16((const uint8_t *)"123456789", 9) == 0x29B1, "vecteur CRC connu");
}
...
static void test_build_parse(void)
{
  ...
  CHECK(len == 5 + 4 + 2, "longueur de trame");
...
static void test_corruption(void)
{
  ...
  frame[6] ^= 0xFF;                  
  uint8_t seq, cmd, status; const uint8_t *data; uint16_t dlen;
  CHECK(pt_frame_parse(frame, (size_t)len, &seq, &cmd, &status, &data, &dlen) == -1, "CRC corrompu -> rejet");
}
