...
static uint8_t buf[8 + J2534_MSG_DATA_MAX];  
...
void transport_rx_byte(uint8_t b)
{
  ...
  case S_LEN1: len |= (uint16_t)b << 8;
         if (len > J2534_MSG_DATA_MAX) { st = S_SOF; break; }
         st = S_SEQ; break;
...
void proto_dispatch(const proto_frame_t *f)
{
  ...
  case CMD_START_FILTER: {
    ...
    if (f->len < 20) { status = J2534_ERR_NULL_PARAMETER; break; }
    uint32_t fid = 0;
    status = j2534_start_filter(rd32(p), (uint8_t)rd32(p + 4),
                  rd32(p + 8), rd32(p + 12), rd32(p + 16), &fid);
    memcpy(out, &fid, 4); out_n = 4; break; }
